...
export async function handleGMLCodeBlockMessages(message: Message<boolean>) {
  ...
  const results = await Promise.all(codes.map(code => new Promise(async resolve => {
    ...
    }) as Promise<{ success: boolean; message: string; exec?: any; }>);
    ...
    const unlinkResult = await unlink(filePath).catch(() => null);
    if (unlinkResult === null) {
      resolve({ success: false, message: 'Failed to delete file', exec });
      return;
    }
